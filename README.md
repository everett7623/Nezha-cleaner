...
这是一个专门用于彻底清理哪吒探针的安全工具。v2.1 支持通过交互式菜单选择卸载 Agent（被控端）、Dashboard（主控端）或同时卸载两者 — 每项都有独立的安全清理流程，新增媒体文件保护。

*This is a specialized and safe tool for completely removing Nezha monitoring components. v2.1 provides an interactive menu to uninstall the Agent (controlled endpoint), Dashboard (control panel), or both — each with its own dedicated safety pipeline, now with media file protection.*
...
- 🆕 **双模式菜单** - v2.1 交互式菜单，可选卸载 Agent / Dashboard / 两者
- 🛡️ **安全可靠** - v2.1 四层 Docker 纵深防御 + 媒体文件保护 + 精准叶子目录保护 + `safe_remove()` 统一安全删除，杜绝误删
- 🔍 **智能路径追踪** - 通过 `/proc/{pid}/exe` 和 systemd 服务文件自动发现非标准安装路径
- 🌐 **双语支持** - 同时显示中文和英文提示信息
...
### v2.1 不会删除以下内容 (v2.1 Will NOT Delete)
...
| 功能 | v1.0 | v1.1 | v1.2 | v1.3 | v1.4 | v2.0 | v2.1 |
|------|------|------|------|------|------|------|------|
| 基础清理功能 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 精确匹配nezha文件 | ⚠️ 部分 | ✅ 完全 | ✅ 完全 | ✅ 完全 | ✅ 完全 | ✅ 完全 | ✅ 完全 |
| 避免误删系统文件 | ❌ 有风险 | ✅ 安全 | ✅ 安全 | ✅ 安全 | ✅ 安全 | ✅ 安全 | ✅ 安全 |
| 二次确认机制 | ❌ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 媒体/文档文件保护 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ **27种格式** |
| Docker容器精确识别 | ⚠️ 宽泛 | ✅ 精确 | ✅ 精确 | ✅ 精确 | ✅ **原生过滤+验证** | ✅ **四层防御** | ✅ **四层防御** |
| 智能路径追踪 | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 系统目录保护 | ❌ | ❌ | ✅ | ✅ (精准叶子级) | ✅ (精准叶子级) | ✅ (精准叶子级) | ✅ (精准叶子级) |
| curl-pipe 交互支持 | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ |
| 统一安全删除 `safe_remove()` | ❌ | ❌ | ❌ | ✅ | ✅ (全覆盖) | ✅ (全覆盖) | ✅ (全覆盖) |
| Docker 内部存储排除 | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ |
...
A: v2.1 版本已加入四层 Docker 纵深防御体系 + 媒体文件保护，Dashboard 模式会自动跳过 Agent 容器（反之亦然）。Docker 内部存储目录被显式排除，删除前通过 `docker inspect` 逐容器验证，图片/文档等用户文件即使文件名含"nezha"也会自动跳过。其他软件完全不受影响。
</details>
