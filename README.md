...
这是一个专门用于彻底清理哪吒探针的安全工具。v2.0 支持通过交互式菜单选择卸载 Agent（被控端）、Dashboard（主控端）或同时卸载两者 — 每项都有独立的安全清理流程，确保不误删其他 Docker 容器或系统文件。

*This is a specialized and safe tool for completely removing Nezha monitoring components. v2.0 provides an interactive menu to uninstall the Agent (controlled endpoint), Dashboard (control panel), or both — each with its own dedicated safety pipeline, ensuring other Docker containers and system files are never touched.*

---

## 🔥 重要更新 (Important Update)

### 🆕 v2.0 Dashboard + Agent 双模式版 (Latest)

**交互式菜单，支持安全卸载 Dashboard（主控端）和 Agent（被控端）！**

**Interactive menu — safely uninstall Dashboard (Control Panel) and/or Agent (Controlled Endpoint)!**

**v2.0 更新内容 (What's New):**
- 🆕 **交互式主菜单**: 启动时选择 1)卸载Agent 2)卸载Dashboard 3)同时卸载两者 4)退出
- 🆕 **Dashboard 安全清理**: 12步独立流程，覆盖裸机 + Docker Compose + 独立容器部署
- 🆕 **Docker 镜像删除**: 容器清理后二次确认删除 `ghcr.io/nezhahq/nezha` 等镜像
- 🆕 **容器归属分类**: Agent 模式跳过 Dashboard 容器，Dashboard 模式跳过 Agent 容器
- 🛡️ **四层 Docker 纵深防御**: 原生过滤 → 归属分类 → docker inspect 验证 → 镜像二次确认
...
- 🆕 **双模式菜单** - v2.0 交互式菜单，可选卸载 Agent / Dashboard / 两者
- 🛡️ **安全可靠** - v2.0 四层 Docker 纵深防御 + 精准叶子目录保护 + `safe_remove()` 统一安全删除，杜绝误删
- 🔍 **智能路径追踪** - 通过 `/proc/{pid}/exe` 和 systemd 服务文件自动发现非标准安装路径
- 🌐 **双语支持** - 同时显示中文和英文提示信息
...
### v2.0 不会删除以下内容 (v2.0 Will NOT Delete)
...
### v2.0 (2026-06) - 🆕 Dashboard + Agent 双模式版 (Latest)
...
| 功能 | v1.0 | v1.1 | v1.2 | v1.3 | v1.4 | v2.0 |
|------|------|------|------|------|------|------|
| 基础清理功能 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 精确匹配nezha文件 | ⚠️ 部分 | ✅ 完全 | ✅ 完全 | ✅ 完全 | ✅ 完全 | ✅ 完全 |
| 避免误删系统文件 | ❌ 有风险 | ✅ 安全 | ✅ 安全 | ✅ 安全 | ✅ 安全 | ✅ 安全 |
| 二次确认机制 | ❌ | ✅ | ✅ | ✅ | ✅ | ✅ |
| Docker容器精确识别 | ⚠️ 宽泛 | ✅ 精确 | ✅ 精确 | ✅ 精确 | ✅ **原生过滤+验证** | ✅ **四层防御** |
| 智能路径追踪 | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ |
| 系统目录保护 | ❌ | ❌ | ✅ | ✅ (精准叶子级) | ✅ (精准叶子级) | ✅ (精准叶子级) |
| curl-pipe 交互支持 | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ |
| 统一安全删除 `safe_remove()` | ❌ | ❌ | ❌ | ✅ | ✅ (全覆盖) | ✅ (全覆盖) |
| Docker 内部存储排除 | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ |
...
A: v2.0 版本已加入四层 Docker 纵深防御体系，且 Dashboard 模式会自动跳过 Agent 容器（反之亦然）。Docker 内部存储目录被显式排除，删除前还会通过 `docker inspect` 逐容器验证，删除镜像需要二次确认。其他软件完全不受影响。
</details>
