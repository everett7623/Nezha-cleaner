...
![Runs](https://visitor-badge.laobi.icu/badge?page_id=everett7623.nezha-cleaner&left_text=runs)
...
这是一个专门用于彻底清理哪吒探针的安全工具。v2.2 支持通过交互式菜单选择卸载 Agent（被控端）、Dashboard（主控端）或同时卸载两者 — 每项都有独立的安全清理流程，新增使用统计计数器及多项 Bug 修复。

*This is a specialized and safe tool for completely removing Nezha monitoring components. v2.2 provides an interactive menu to uninstall the Agent (controlled endpoint), Dashboard (control panel), or both — each with its own dedicated safety pipeline, now with usage statistics counter and multiple bug fixes.*
...
**Added usage statistics counter + 7 bug fixes!**

**v2.2 更新内容 (What's New):**
- 📊 **使用统计计数器**: 主菜单显示累计运行次数 (visitor-badge.laobi.icu)
- 🐛 **Bug修复**: trap覆盖、符号链接保护、ExecStart前缀、crontab边界、竞态条件、Docker匹配

### 📋 v2.1 安全增强版

**修复致命bug：全局扫描不再误删用户媒体/文档文件！**

**Critical bug fix: global scan no longer deletes user media/document files!**

**v2.1 更新内容 (What's New):**
- 🐛 **致命修复**: `safe_remove()` 新增文件类型保护——自动跳过图片/文档/媒体文件（`.png`, `.jpg`, `.pdf`, `.md`, `.mp4` 等 27 种扩展名），防止误删用户文章配图等内容
- 📝 **文档更新**: CLAUDE.md 和 README 同步更新至 v2.1

### 📋 v2.0 Dashboard + Agent 双模式版

**交互式菜单，支持安全卸载 Dashboard（主控端）和 Agent（被控端）！**

**v2.0 更新内容:**
- 🆕 交互式主菜单: 启动时选择 1)卸载Agent 2)卸载Dashboard 3)同时卸载两者 4)退出
- 🆕 Dashboard 安全清理: 12步独立流程，覆盖裸机 + Docker Compose + 独立容器部署
- 🆕 Docker 镜像删除 + 容器归属分类 + 四层 Docker 纵深防御
- 🔧 函数化重构: `cleanup_agent()` 和 `cleanup_dashboard()` 独立函数，变量隔离
...
- 🆕 **双模式菜单** - v2.2 交互式菜单，可选卸载 Agent / Dashboard / 两者
- 🛡️ **安全可靠** - v2.2 四层 Docker 纵深防御 + 媒体文件保护 + 精准叶子目录保护 + `safe_remove()` 统一安全删除，杜绝误删
- 🔍 **智能路径追踪** - 通过 `/proc/{pid}/exe` 和 systemd 服务文件自动发现非标准安装路径
- 🌐 **双语支持** - 同时显示中文和英文提示信息
- 🧹 **彻底清理** - Agent 和 Dashboard 各执行 12 步流程全面清理
- 🔄 **交互式操作** - 对于重要操作会请求用户确认，curl-pipe 场景下正常工作
- 🎨 **彩色输出** - 使用不同颜色区分信息类型，提高可读性
- 🐳 **Docker支持** - 检测并提供清理相关Docker容器和镜像的选项
...
### 使用中 (During Use)
- 🔔 脚本有交互式步骤，需要人工确认某些操作
- 🔔 步骤7会列出所有包含"nezha"的文件，请仔细检查后再确认删除（媒体/文档文件会自动跳过）
...
### v2.2 不会删除以下内容 (v2.2 Will NOT Delete)

- ✅ Docker 内部存储 (`/var/lib/docker`, `/var/lib/containerd`) — 被全局扫描显式排除
- ✅ 用户媒体/文档文件 (`.png`, `.jpg`, `.pdf`, `.md`, `.mp4` 等 27 种格式) — 即使文件名含"nezha"也自动跳过
- ✅ 其他 Docker 容器 — 四层防御（原生过滤 → 归属分类 → `docker inspect` 验证 → 镜像二次确认），非 nezha 容器自动跳过
- ✅ Dashboard 模式下 Agent 容器 — 容器名含 `nezha-agent` 的自动跳过
- ✅ Agent 模式下 Dashboard 容器 — 原生过滤仅匹配 `*nezha-agent*`
- ✅ `ssh-agent` - SSH密钥代理
...
脚本使用以下严格的匹配模式：
- Dashboard 进程：`nezha-dashboard`
- Agent 进程：`nezha-agent`
- Dashboard 服务：`nezha-dashboard.service` 或 `nezha.service`（排除 Agent）
- Agent 服务：`nezha-agent.service` 或 `nezha.service`
- 文件：文件名必须包含 `nezha`（Docker/containerd 内部存储已被排除）
- Dashboard Cron：`nezha-dashboard` 或 `/nezha/dashboard`
- Agent Cron：`nezha-agent` 或 `/nezha/`
- Agent Docker 容器：Docker 原生 `--filter "name=*nezha-agent*"` 过滤
- Dashboard Docker 容器：`--filter "name=*nezha*"` + 分类跳过 `*nezha-agent*`
- Docker 镜像：`grep -iE "nezha"` 匹配，删除前二次确认
- **Docker 纵深防御**: 原生过滤 → 归属分类 → `docker inspect` 验证 → 镜像二次确认

---

## 🔄 版本历史 (Version History)

### v2.2 (2026-06) - 📊 统计+修复版 (Latest)
- 📊 **新增**: 使用统计计数器 — 主菜单显示累计运行次数 (visitor-badge.laobi.icu)
- 🐛 **修复**: trap覆盖 — Both模式下临时文件不再泄漏（全局数组追踪）
- 🐛 **修复**: 符号链接保护 — `safe_remove()` 检测指向保护目录的 symlink
- 🐛 **修复**: ExecStart前缀 — 扩展剥离字符类支持 `:` 修饰符
- 🐛 **修复**: crontab边界 — 空 crontab 使用 `crontab -r`，无 crontab 命令时跳过
- 🐛 **修复**: 竞态条件 — pgrep 路径追踪增加 `/proc/$pid` 存在性检查
- 🐛 **修复**: Docker匹配 — Agent模式 grep 收窄为 `nezha-agent`，不再误匹配 Dashboard
- 📝 **文档**: README 和脚本版本号更新至 v2.2

### v2.1 (2026-06) - 🛡️ 安全增强版
- 🐛 **致命修复**: `safe_remove()` 新增文件类型保护 — 自动跳过图片/文档/媒体文件（27 种扩展名）
- 🐛 **修复**: 全局 find 扫描不再误删用户文章配图等非哪吒监控组件的用户内容
- 📝 **文档**: CLAUDE.md 和 README 同步更新至 v2.1

### v2.0 (2026-06) - 🆕 Dashboard + Agent 双模式版
- 🆕 **新增**: 交互式主菜单 — 可选 Agent、Dashboard 或同时卸载
- 🆕 **新增**: Dashboard (主控端) 12步安全清理流程
- 🆕 **新增**: Docker 镜像删除 — 容器清理后二次确认删除 nezha 镜像
- 🆕 **新增**: 容器归属分类 — Agent/Dashboard 模式互不干扰
- 🛡️ **升级**: Docker 纵深防御从三层升级为四层（+ 镜像二次确认）
- 🔧 **重构**: 清理逻辑函数化，变量隔离，支持 Both 模式
- 📝 **文档**: CLAUDE.md 和 README 同步更新至 v2.0

### v1.4 (2026-06) - 🛡️ Docker 安全版
...
| 功能 | v1.0 | v1.1 | v1.2 | v1.3 | v1.4 | v2.0 | v2.1 | v2.2 |
|------|------|------|------|------|------|------|------|------|
| 基础清理功能 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 精确匹配nezha文件 | ⚠️ 部分 | ✅ 完全 | ✅ 完全 | ✅ 完全 | ✅ 完全 | ✅ 完全 | ✅ 完全 | ✅ 完全 |
| 避免误删系统文件 | ❌ 有风险 | ✅ 安全 | ✅ 安全 | ✅ 安全 | ✅ 安全 | ✅ 安全 | ✅ 安全 | ✅ 安全 |
| 二次确认机制 | ❌ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 媒体/文档文件保护 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ **27种格式** | ✅ **27种格式** |
| Docker容器精确识别 | ⚠️ 宽泛 | ✅ 精确 | ✅ 精确 | ✅ 精确 | ✅ **原生过滤+验证** | ✅ **四层防御** | ✅ **四层防御** | ✅ **四层防御** |
| 智能路径追踪 | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 系统目录保护 | ❌ | ❌ | ✅ | ✅ (精准叶子级) | ✅ (精准叶子级) | ✅ (精准叶子级) | ✅ (精准叶子级) | ✅ (精准叶子级) |
| curl-pipe 交互支持 | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 统一安全删除 `safe_remove()` | ❌ | ❌ | ❌ | ✅ | ✅ (全覆盖) | ✅ (全覆盖) | ✅ (全覆盖) | ✅ (全覆盖) |
| Docker 内部存储排除 | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ |
| docker inspect 逐容器验证 | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ |
| Dashboard (主控端) 卸载 | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ |
| Docker 镜像删除 | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ |
| 容器归属分类 (Agent/Dashboard) | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ |
| 交互式主菜单 | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ |
| 使用统计计数器 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ **visitor-badge** |
| 推荐使用 | ❌ | ⚠️ 可用 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ **强烈推荐** |
...
A: v2.2 版本已加入四层 Docker 纵深防御体系 + 媒体文件保护，Dashboard 模式会自动跳过 Agent 容器（反之亦然）。Docker 内部存储目录被显式排除，删除前通过 `docker inspect` 逐容器验证，图片/文档等用户文件即使文件名含"nezha"也会自动跳过。其他软件完全不受影响。
</details>
...
<details>
<summary><b>Q: Dashboard 和 Agent 有什么区别？该选哪个？</b></summary>

A: Dashboard（主控端）是监控面板，通常部署在一台服务器上；Agent（被控端）安装在被监控的每台 VPS 上。想卸载监控面板选 2，想卸载某台 VPS 上的探针选 1，想全部清除选 3。脚本会自动识别并根据选择跳过不属于当前模式的容器。
</details>
